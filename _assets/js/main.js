...
jQuery(function (a) {
  var g = a("html"),
    h = a(window);
  g.removeClass("no-js");
  var i = a(".site-nav--primary"),
    j = 0;
  j = a(".site-nav--primary").offset().top;
  var k = a(".site-nav--categories"),
    l = 0;
  (l = a(".site-nav--categories").offset().top),
    h.on("load scroll resize", function () {
      i.toggleClass("site-nav--fixed", h.scrollTop() > j),
        h.width() > 767 &&
          (k.toggleClass("site-nav--fixed", h.scrollTop() > l - 60),
          g.toggleClass("site-nav-all--fixed", h.scrollTop() > l - 60)),
        a("#calendar").hasClass("active") &&
          (a("#calendar").slideUp(200),
          a("#calendar,button.site-nav__calendar-toggle").removeClass(
            "active"
          )),
        a(".section--search").hasClass("active") &&
          (a(".section--search").slideUp(200),
          a(".section--search, .site-nav__search-toggle").removeClass(
            "active"
          )),
        a(".new-nav-wrap").hasClass("active") &&
          (a(".site-nav__categories-toggle, .new-nav-wrap").removeClass(
            "active"
          ),
          a(".new-nav-wrap").hide());
    }),
    a("button.site-nav__categories-toggle").click(function (b) {
      h.width() < 1200 &&
        (a(".search").removeClass("active"),
        a(".calendar").removeClass("active"),
        a(".new-nav-wrap").slideToggle(250).toggleClass("active"),
        a(this).toggleClass("active"),
        a("button.site-nav__calendar-toggle").removeClass("active"),
        a("#calendar").slideUp(200),
        a("#calendar").removeClass("active")),
        b.preventDefault();
    }),
    a("button.site-nav__calendar-toggle").click(function (b) {
      a(
        ".site-nav__list--channels, .site-nav__list--categories, .site-nav__title, .site-nav__categories-toggle"
      ).removeClass("active"),
        a(".new-nav-wrap").hide(),
        a(this).toggleClass("active"),
        a(".site-nav__search-toggle").removeClass("active"),
        a(".section--search").slideUp(200),
        a(".section--search").removeClass("active"),
        a("#calendar").slideToggle(200),
        a("#calendar").toggleClass("active"),
        b.preventDefault();
    }),
    h.width() > 1024 &&
      a(".site-nav__search-toggle").click(function (b) {
        a(".site-nav__list--channels").removeClass("active"),
          a(this).toggleClass("active"),
          a("button.site-nav__calendar-toggle").removeClass("active"),
          a("#calendar").slideUp(200),
          a("#calendar").removeClass("active"),
          a(".section--search").slideToggle(200),
          a(".section--search").toggleClass("active"),
          b.preventDefault();
      }),
    a("button.full-calendar__toggle").click(function (b) {
      a("#full-calendar").slideToggle(),
        a(this).toggleClass("active"),
        b.preventDefault();
    }),
    a("select.search__select").dropkick({
      mobile: !0,
      theme: "search-select",
      change: function (b) {
        a(this).dropkick("theme", b);
      },
    }),
    a("select.calendar-select").dropkick({
      mobile: !0,
...
(function () {
  async function a() {
    ...
    if (2 < e.length) {
      ...
      const g = {};
      for (let b of a)
        g.hasOwnProperty(b.category) || (g[b.category] = []),
          g[b.category].push(b);
      let h = "";
      for (let a in g) {
        const b = g[a];
        h += `<p class="search-result__category">${a}</p><ul class="search-result__list">`;
        for (let a of b) {
          const { id: b, label: c, category: g } = a;
          h += `<li class="search-result__item" data-href="${`${f}/${b}`}" data-label="${c}"><span class="search-result__label">${d(
            e,
            c
          )}</span></li>`;
        }
  ...
  function b(a) {
    const b = this.dataset.target,
      c = document.querySelector(b),
      d = c.querySelectorAll(".search-result__item"),
      e = [...d].findIndex((a) => a.classList.contains("selected"));
    let f = 0;
    if ("ArrowUp" === a.key) f = -1 === e || 0 === e ? d.length - 1 : e - 1;
    else if ("ArrowDown" === a.key)
      f = -1 === e || e === d.length - 1 ? 0 : e + 1;
    else return;
    0 <= e && d[e].classList.remove("selected"),
      d[f] &&
        (d[f].classList.add("selected"), (this.value = d[f].dataset.label));
  }
  async function c(a) {
    try {
      const b = await fetch(`${g}${a}`),
        c = b.json();
      return c;
    } catch (a) {
      return [];
  ...
  function d(a, b) {
    const c = a.length,
      ...
      e = b.search(d);
    if (0 > e) return b;
    const f = b.substring(0, e),
      g = b.substring(e, e + c),
      h = b.substring(e + c);
    return `${f}<span class="search-result__highlight">${g}</span>${h}`;
  }
  function e(a, b, c) {
    let d;
  ...
  const f = "https://www.onthisday.com",
    g = `${"https://www.onthisday.com"}/_assets/ac.php?term=`,
    h = document.querySelectorAll("form[action$='/search.php']");
  for (let a = 0; a < h.length; ++a) {
    const b = h[a],
      c = document.createElement("div"),
      d = "search-result-container-" + (a + 1);
    (c.id = d), c.classList.add("search-result__container"), b.after(c);
    const e = b.querySelector(".search__text");
    e && e.setAttribute("data-target", `#${d}`);
  }
  const i = document.querySelectorAll(".search__text");
  for (let c of i)
    c.addEventListener("input", e(a, 300)), c.addEventListener("keyup", b);
  document.body.addEventListener("click", function (a) {
    if (a.target.classList.contains("search-result__item")) {
      const b = a.target.dataset.href,
        c = a.target.dataset.label,
        d = a.target.parentElement.parentElement,
        e = `#${d.id}`,
        f = document.querySelector(`[data-target="${e}"]`);
      (f.value = c), (window.location = b);
    }
    const b = document.querySelectorAll(".search-result__container");
    for (let c of b)
      c === a.target || c.contains(a.target) || (c.style.display = "none");
  }),
    document.body.addEventListener("keydown", function (a) {
      if ("Escape" === a.key) {
        const a = document.querySelectorAll(".search-result__container");
        for (let b of a) b.style.display = "none";
      }
    });
})();
